...
const fontProperties: Record<FontName, number> = {
  'Swell Braille': 29,
  'California Braille': 29,
  Braille29: 29,
};

export type FontName = 'Braille29' | 'California Braille' | 'Swell Braille';
...
function updateFontAcross(obj: any, userFont: string, userFontSize: number) {
  Object.keys(obj).forEach((key) => {
    if (typeof obj[key] === 'object' && !Array.isArray(obj[key])) {
      ...
      if (key === 'labelFont' || key === 'titleFont' || key === 'font' || key === 'subtitleFont') {
        // Check if the key is related to font settings
        obj[key] = userFont;
      }
...
function updateFont(userSpec: any, defaultSpec: any) {
  ...
  if (userSpec.config && userSpec.config.text) {
    ...
    const userFontSize = fontProperties[userFont as FontName] || 29;
...
function updateDefault(userSpec: any, defaultSpec: any) {
  ...
  if (defaultSpec.mark === 'bar' || defaultSpec.mark.type === 'bar') {
    if (userSpec.encoding.xOffset || userSpec.encoding.yOffset) defaultSpec = updateGroupedBarAxis(quantAxis, defaultSpec);
  }
