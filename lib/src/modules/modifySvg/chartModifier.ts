...
import { embedFontFromSpec } from '../utils/embedFonts';
...
async function modifySvg(result: any, spec: any) {
  // Get the SVG element from the vega-embed result and initialize patterns
  const svgElement = result.view.container().querySelector('svg');
  if (svgElement) {
    initSvgPatterns(svgElement);
  }
  ...
  // Embed the appropriate font based on the specification
  if (svgElement) {
    embedFontFromSpec(svgElement, spec);
  }
}
